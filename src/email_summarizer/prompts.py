...
def get_email_summarizer_prompt() -> ChatPromptTemplate:
    """
    【优化版 v2】Prompt，用于学生邮箱，增强了对金融通知的识别。
    """
...
1.  **精准分类**: 从以下类别中选择最合适的一个：
    ...
    * `金融通知`: (**重要**: 银行交易、账单提醒、账户变动 - **非广告**)
    * `普通学业`: (老师回复、一般教学通知、资料分享)
    * `一般招聘`: (招聘宣讲、职位推送、招聘会)
    * `校园活动`: (社团、讲座、非学业活动通知)
    * `个人社交`: (同学朋友邮件)
    * `推广广告`: (无关推广、机构宣讲、营销邮件 - **包括银行产品推广**)
...
2.  **准确评级**: 根据内容和分类，给出1-5星重要性评级 (5星最高)，依据如下：
    ...
    * ★★☆☆☆: 一般的 `校园活动` 或 `个人社交`。
    * ★☆☆☆☆: `推广广告` 类；不重要的宣讲会或活动。
    * **区分提示**: 金融通知通常包含具体金额、日期、账户尾号等交易细节，而推广广告则介绍理财产品、信用卡优惠等。
